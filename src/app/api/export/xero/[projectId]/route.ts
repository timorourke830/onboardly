...
import { generateXeroCSV, getXeroContentType, getXeroFilename } from "@/lib/export";
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Generate Xero CSV content
    const csvContent = generateXeroCSV(accounts);
    const filename = getXeroFilename(project.name);
    const contentType = getXeroContentType();
    ...
    console.error("Error exporting Xero CSV:", error);
    return NextResponse.json(
